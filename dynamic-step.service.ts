...
async function smartSelect(ctx: any, page: any, selector: string, value: string, opts?: { timeout?: number }) {
  ...
  if (tagName === 'select') {
    ...
    if (isVisible) {
      try { await handle.selectOption({ label: value }, { timeout: Math.min(timeout, 8000) }); return; } catch {}
      try { await handle.selectOption(value, { timeout: Math.min(timeout, 8000) }); return; } catch {}
    }
