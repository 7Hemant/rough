...
export class DynamicStepService {
  ...
  async executeWithDynamicFields(opts: {
    ...
    // ---- ELEMENT-SCOPED repair helpers -------------------------------------
    // CRITICAL: when Permanent and Communication address sections are BOTH
    // visible, a field's selector (e.g. [name="City"]) can match TWO elements.
    //  - `ctx.$(selector)` returns the FIRST match → the Permanent field,
    //    which is never empty → the check reported "all good" while the
    //    Communication copy sat on "Select".
    //  - smartFill/smartSelect resolve the first VISIBLE match → they
    //    repaired the Permanent field and never touched the empty one.
    // So verification scans EVERY match, and repair acts on the EXACT element
    // HANDLE that was found empty — never re-resolving by selector.

    /** Apply a select value on a specific element handle, retrying while
     *  AJAX-loaded options arrive. Returns true if the option was applied. */
    const applySelectOnHandle = async (handle: any, value: string, timeoutMs: number): Promise<boolean> => {
      const tryApply = () =>
        handle
          .evaluate((el: any, val: string) => {
            ...
            if (jq) {
              try {
                const $el = jq(el);
                $el.val(match.value).trigger('change');
                try { $el.trigger('change.select2'); } catch {}
                if (typeof $el.selectpicker === 'function') { try { $el.selectpicker('refresh'); } catch {} }
                try { $el.trigger('chosen:updated'); } catch {}
              } catch {}
            }
            return true;
    ...
    /** Fill a specific element handle; falls back to JS assignment for
     *  readonly/disabled (greyed "Same as Permanent" copies) inputs. */
    const applyFillOnHandle = async (handle: any, value: string): Promise<void> => {
      try { await handle.fill(value, { timeout: 3000 }); } catch {}
      const cur = (await handle.inputValue().catch(() => '')) || '';
      if (cur.trim() === '' && value.trim() !== '') {
        await handle
          .evaluate((node: any, val: string) => {
            ...
            node.dispatchEvent(new Event('change', { bubbles: true }));
            const jq = (window as any).jQuery;
            if (jq) { try { jq(node).val(val).trigger('change'); } catch {} }
            if (wasReadonly) node.setAttribute('readonly', 'readonly');
          }, value)
          .catch(() => {});
      }
    ...
    /** Diagnostic: what options does this select actually contain right now? */
    const describeSelectOptions = async (handle: any): Promise<string> =>
      handle
        .evaluate((el: any) => {
          if (!el || el.tagName !== 'SELECT') return 'not a <select>';
          const opts = Array.from(el.options || []).map(
            (o: any) => `"${(o.textContent || '').trim()}"(${o.getAttribute ? o.getAttribute('value') : o.value})`,
          );
          return `${opts.length} option(s): ` + opts.slice(0, 8).join(', ') + (opts.length > 8 ? ' …' : '');
        })
        .catch(() => 'unreadable');

    /**
     * Re-fire `change` on every registered select that currently HOLDS a real
     * value. This re-triggers the parent→child AJAX (State → City) so an empty
     * dependent dropdown whose OPTION LIST was wiped by the cascade gets its
     * options reloaded before we retry selecting into it.
     */
    const kickCascades = async (currentStep: number) => {
      logger.wait(currentStep, 'Re-triggering parent dropdown change events to reload dependent options');
      for (const f of filledRegistry) {
        if (f.kind !== 'select') continue;
        try {
          const els = await f.ctx.$$(f.selector);
          for (const el of els) {
            const holdsValue = await el
              .evaluate((n: any) => {
                if (!n.tagName || n.tagName !== 'SELECT' || !n.value) return false;
            ...
            if (holdsValue) {
              await el
                .evaluate((n: any) => {
                  ...
                  const jq = (window as any).jQuery || (window as any).$;
                  if (jq) { try { jq(n).trigger('change'); } catch {} }
                })
                .catch(() => {});
            }
          }
      ...
      await waitForAppReady(page, { timeout: 12000, graceMs: 400, quietChecks: 2 });
    };

    /**
     * One verification pass over the registry — checks EVERY element matching
     * each registered selector (Permanent + Communication copies) and repairs
     * the exact handle that is empty.
     * Returns the NUMBER of repairs performed (0 = clean pass).
     */
    const reapplyClearedFills = async (currentStep: number): Promise<number> => {
      let reapplied = 0;
      for (const f of filledRegistry) {
        try {
          const els = await f.ctx.$$(f.selector);
          for (const el of els) {
            if (f.kind === 'fill') {
              ...
              if (cur !== null && cur.trim() === '' && f.value.trim() !== '') {
                // Only repair rendered fields — hidden inputs in inactive
                // wizard panels are legitimately empty. Disabled-but-visible
                // (greyed copy) fields ARE repaired.
                const rendered = await el
                  .evaluate((n: any) => !!(n.offsetWidth || n.offsetHeight || n.getClientRects().length))
                  .catch(() => false);
                if (!rendered) continue;
                logger.warn(currentStep, `Input cleared by the page — re-applying "${f.value.slice(0, 30)}" on the exact element`, { details: f.selector });
                await applyFillOnHandle(el, f.value);
                reapplied++;
              }
            } else {
              ...
              if (isEmpty && f.value.trim() !== '') {
                // Rendered = the select itself or its enhanced-widget wrapper
                // is visible (enhanced selects hide the native element).
                const rendered = await el
                  .evaluate((n: any) => {
                    const vis = (x: any) => !!(x && (x.offsetWidth || x.offsetHeight || x.getClientRects().length));
                    if (vis(n)) return true;
                    const wrap = n.closest && (n.closest('.bootstrap-select') || n.closest('.select2') || n.closest('.chosen-container'));
                    return !!(wrap && vis(wrap));
                  })
                  .catch(() => false);
                if (!rendered) continue;

                logger.warn(currentStep, `Dropdown lost its selection — re-selecting "${f.value}" on the exact empty element`, { details: f.selector });
                let ok = await applySelectOnHandle(el, f.value, 6000);
                if (!ok) {
                  // The matching option doesn't exist — the cascade wiped the
                  // option list. Re-trigger parent dropdowns to reload it.
                  logger.warn(currentStep, `Option "${f.value}" not present — ${await describeSelectOptions(el)}. Re-triggering cascades to reload options.`, { details: f.selector });
                  await kickCascades(currentStep);
                  ok = await applySelectOnHandle(el, f.value, 12000);
                }
                if (!ok) {
                  logger.error(currentStep, `Could not restore "${f.value}" — select still has ${await describeSelectOptions(el)}`, { details: f.selector });
                }
                reapplied++;
              }
            }
      ...
      if (reapplied > 0) {
        // Repairs fired change handlers — let their cascades finish before the
        // caller re-verifies.
        await waitForAppReady(page, { timeout: 12000, graceMs: 300, quietChecks: 2 });
      }
      return reapplied;
    };

    /**
     * CASCADE-RESET FIX: verify/repair until a full CLEAN pass.
     *  - Waits for the app to go QUIET before verifying (never verify
     *    mid-reset).
     *  - Scans ALL selector matches, repairs exact element handles.
     *  - Loops until a pass finds nothing to repair (a repair's own cascade
     *    can reset another field).
     */
    const ensureFieldsIntact = async (currentStep: number) => {
      await waitForAppReady(page, { timeout: 10000, graceMs: 300, quietChecks: 2 });

      for (let pass = 0; pass < 5; pass++) {
        const n = await reapplyClearedFills(currentStep);
        if (n === 0) return; // clean pass — every field holds its value
      }
      logger.warn(currentStep, 'Field integrity check did not stabilize after 5 passes — proceeding anyway');
    };
        ...
        try {
          ...
          switch (field.actionType) {
            case 'fill':
              ...
              break;
            case 'click': {
              // CASCADE-RESET FIX: resolve the button FIRST so the moment the
              // integrity check passes we click IMMEDIATELY — no long
              // element-wait in between for a late AJAX response to sneak into
              // and wipe a dependent dropdown (Communication City).
              logger.wait(stepNum, `Waiting for "${field.label}" to be visible & clickable`, { selector: resolvedSel });
              const clickEl = await firstVisibleElement(ctx, resolvedSel, 20000);

              // Verify every previously filled input still holds its value AND
              // every previously selected dropdown is still selected — LOOP
              // until a clean pass (handles cascade resets, copy-toggle races,
              // and repairs that themselves reset other fields).
              await ensureFieldsIntact(stepNum);

              try { await clickEl.scrollIntoViewIfNeeded(); } catch {}
              await clickEl.click({ timeout: 10000 });
              await waitForPageStable(page, { timeout: 10000 });
            ...
            case 'press':
              // Enter frequently submits the current form — run the same
              // integrity guard as a Submit click before pressing it.
              if ((value || 'Enter') === 'Enter') {
                await ensureFieldsIntact(stepNum);
              }
              ...
              // Enter may have advanced the screen — same registry reset as click.
              if ((value || 'Enter') === 'Enter') {
                filledRegistry.length = 0;
              }
              break;
            ...
            case 'clickIfVisible': {
              ...
              try {
                ...
                if (el) {
                  // Same looped integrity guard as regular clicks.
                  await ensureFieldsIntact(stepNum);
