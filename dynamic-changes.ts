...
/**
 * Smart wait: DOM parsed + app quiet. SPEED-TUNED: networkidle removed — for a
 * jQuery app, jQuery.active is a stricter AND faster signal (networkidle costs
 * 500ms minimum and stalls on pages with background polling).
 */
...
/**
 * VALUE-CLEARING FIX: close a datepicker popup ONLY if one is actually open.
 *
 * The old code pressed Escape unconditionally after every fill. That is what
 * was wiping field values: jQuery inputmask RESTORES THE ORIGINAL (empty)
 * VALUE on Escape, and daterangepicker CANCELS AND REVERTS on Escape. So
 * masked inputs (phone, PAN, amount) and date-range fields looked filled but
 * were silently reverted — noticed only when Next/Submit validated.
 *
 * Now Escape is pressed only when a calendar/picker widget is visibly open,
 * which is the one case where it's needed (to stop the popup blocking later
 * steps). Plain and masked inputs are never sent Escape.
 */
async function closeDatepickerIfOpen(page: any) {
  try {
    ...
    if (pickerOpen) {
      // Click a neutral spot first (closes most pickers without touching the
      // input's value), then Escape as backup for pickers that ignore clicks.
 ...
 *    masks accept. No Escape is ever sent to these fields (see
 *    closeDatepickerIfOpen — Escape makes inputmask revert to empty).
...
async function smartFill(ctx: any, page: any, selector: string, value: string, opts?: { timeout?: number }) {
  ...
  // Close a datepicker popup ONLY if one actually opened. Never send Escape
  // to the input itself — inputmask/daterangepicker revert values on Escape.
 ...
 * so the NEXT dependent field's data is loaded before its step runs. This is
 * the ONE place the heavier wait remains — cascades genuinely need it.
 */
...
export class DynamicStepService {
  ...
  async executeWithDynamicFields(opts: {
    ...
    // VALUE-CLEARING FIX (safety net): registry of every filled field in the
    // current screen. Right before any click (Next/Submit), each registered
    // field is re-checked — if a widget/validation cleared its value, it is
    // re-applied before the click proceeds. Cleared on navigation.
    const filledRegistry: { ctx: any; selector: string; value: string }[] = [];
    ...
    try {
      ...
      for (const field of fields) {
        ...
        try {
          // Lean pre-step readiness check: no grace, single passing check —
          // near-instant when the app is idle, still blocks while AJAX or a
          // spinner is active (the heavy cascade wait lives in smartSelect).
          ...
          switch (field.actionType) {
            case 'fill':
              ...
              if (value.trim() !== '') {
                filledRegistry.push({ ctx, selector: resolvedSel, value });
              }
              logger.action(stepNum, `Filled "${field.label}" with value: "${value.slice(0, 50)}${value.length > 50 ? '...' : ''}"`, { selector: resolvedSel });
              break;
            case 'click':
              // VALUE-CLEARING FIX: before Next/Submit clicks, verify every
              // previously filled field still holds its value and re-apply any
              // that a widget/validation cleared (inputmask, daterangepicker).
