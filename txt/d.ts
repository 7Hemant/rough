...
async function driveBootstrapSelect(
  ...
  const wrapperHandle = await handle
    ...
    .catch(() => null);
  const wrapper = wrapperHandle && wrapperHandle.asElement
    ? wrapperHandle.asElement()
    : null;
...
async function driveSelect2(
  ...
  try {
    const openHandle = await handle
      ...
      .catch(() => null);
    const openEl = openHandle && openHandle.asElement
      ? openHandle.asElement()
      : null;
...
async function driveJqueryUiSelectmenu(
  ...
  try {
    const btnHandle = await handle
      .evaluateHandle((el: any) => {
        const sib = el.nextElementSibling;
        if (sib && /ui-selectmenu-button/i.test(sib.className || "")) return sib;
...
async function driveChosen(
  ...
  try {
    const cHandle = await handle
      .evaluateHandle((el: any) => {
        const sib = el.nextElementSibling;
        return sib && /chosen-container/i.test(sib.className || "") ? sib : null;
      })
      .catch(() => null);
 ...
 *
 * Strategy:
 *  1. Plain visible native <select> — Playwright selectOption (by label, then value).
 *  2. Hidden native <select> behind an enhanced widget (Select2 / Chosen /
 *     bootstrap-select / jQuery UI selectmenu) — set the value on the underlying
 *     element, fire native + jQuery change, and call EACH widget's refresh API
 *     (selectpicker('refresh'), change.select2, chosen:updated,
 *     selectmenu('refresh')) so its visible label updates and cascades fire.
 *  3. If the option wasn't present in the native <select> (AJAX/lazy options, or
 *     the recorded text differs) — drive the widget through its UI exactly like
 *     a user, with all menu/option lookups at DOCUMENT level so menus appended
 *     to <body> are found.
 *  4. Fully custom dropdowns — generic open → search → click, exact match first.
 *
 * Multi-step wizard note: when the selector matches multiple <select> elements
 * across step panels, we prefer the one whose widget wrapper (or the element
 * itself) is visible, so we always drive the CURRENT screen's dropdown.
 */
async function smartSelect(
  ...
  if (tagName === "select") {
    ...
    // selectpicker / jQuery UI selectmenu). Match the option by value OR visible
    // text (recorder stores the visible text), set it on the real <select>, then
    // notify BOTH native and jQuery listeners AND call each widget's refresh API
    // so its visible label updates. Cascading dropdowns (State → City) rely on
    // the change handler firing, so this also triggers AJAX loads.
    ...
    // Case 3: the option wasn't found in the native <select> (options load only
    // when the dropdown opens / AJAX, or the recorded text differs slightly) —
    // detect WHICH widget enhances this select and drive it through its UI the
    // way a user does. All option lookups are DOCUMENT-level, so menus appended
    // to <body> (data-container="body", Select2 default, jQuery UI) are found.
