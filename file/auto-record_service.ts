    ...
    (async () => {
      try {
        ...
        await context.addInitScript(() => {
          ...
          // FIX (phantom fields): `change` events also fire for things the user
          // NEVER touched — page-load initialization (`$(el).trigger('change')`),
          // framework value-syncing, and submit-time form resets. Recording those
          // created a field config for every input on the page, and prefilled
          // fields got recorded with an EMPTY value at submit time (the reset),
          // which on replay ran fill("") and wiped the prefill.
          // We now only accept a change as a real user action if:
          //   a) the user directly interacted with that element (trusted
          //      keydown / input / mousedown targeting it), OR
          //   b) for selects/checkboxes/radios only: a trusted user gesture
          //      happened anywhere in the last 1.5s (covers Select2 /
          //      bootstrap-select, where the click lands on widget internals
          //      and the library then triggers change on the hidden <select>).
          let lastUserGestureTime = 0;
          const interactedElements = new WeakSet<any>();
          function markInteraction(e: any) {
            if (!e.isTrusted) return; // programmatic events don't count
            lastUserGestureTime = Date.now();
            let t = e.target;
            if (t && t.nodeType === 1) {
              interactedElements.add(t);
              // Clicking a <label> toggles its associated control — credit the
              // control too, otherwise label-wrapped checkboxes look untouched.
              const lbl =
                t.tagName === "LABEL" ? t : t.closest && t.closest("label");
              if (lbl && lbl.control) interactedElements.add(lbl.control);
            }
          }
          ...
          function recordChange(el: any) {
            ...
            // FIX (record only fields the user acted on):
            const interacted = interactedElements.has(el);
            const recentGesture = Date.now() - lastUserGestureTime < 1500;
            const isTextEntry =
              el.tagName === "TEXTAREA" ||
              (el.tagName === "INPUT" &&
                !/^(checkbox|radio|button|submit|reset|file|image)$/i.test(
                  el.type || "text",
                ));
            if (isTextEntry) {
              // Text inputs: the user must have typed/clicked in THIS element.
              // A submit-button click 200ms ago does NOT make a form-reset
              // change on some other input a user action.
              if (!interacted) return;
            } else {
              // Selects / checkboxes / radios: direct interaction OR a recent
              // gesture (Select2/bootstrap-select route the click through
              // widget internals, then trigger change on the hidden control).
              if (!interacted && !recentGesture) return;
            }
            ...
            // FIX (prefilled fields wiped): an EMPTY value on an element the
            // user never directly touched is a programmatic clear/reset (e.g.
            // the app resetting the form on submit) — never a user choice.
            // Recording it would replay as fill("") and blank out prefills.
            if (!interacted && !recordedValue.trim()) return;
          ...
          document.addEventListener(
            ...
            (e: any) => {
              ...
              // FIX (phantom fields): hovering over a plain form field on the
              // way to something else is not a user action — the change/click
              // listeners capture real interactions with these elements.
          ...
          // FOCUS events — intentionally NOT recorded.
          // FIX (phantom fields): recording every focus meant that merely
          // tabbing through a form, or the page auto-focusing an input,
          // created a step (mapped to `click` on replay) for fields the user
          // never actually used. If the user really acts on the field, the
          // click / change / press listeners capture the real action.
