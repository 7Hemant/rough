...
export class FieldConfigService {
  ...
  async createFromRecordedActions(
    ...
    for (const act of normalizedActions) {
      ...
      // FIX (phantom fields): a bare `focus` (tabbing into / clicking into a
      // field without doing anything) is not a user action — mapping it to a
      // replay `click` created steps for fields the user never used. Applies
      // to legacy recordings made before the recorder stopped emitting focus.
      if (act.action === "focus") continue;
      ...
      // FIX (prefilled fields wiped on replay): never create a `fill` step
      // with an empty value — replay runs fill("") which BLANKS OUT fields the
      // app prefilled. These empty fills come from submit-time form resets /
      // programmatic changes, not from the user.
      if (actionType === "fill" && !(act.value || "").trim()) continue;

      // Same idea for selects: a placeholder-only change ("Select...", "") is
      // page-load initialization, not a choice. If the user later picks a real
      // option, that change creates the field with the real value; if they
      // never touch the dropdown, no field is created at all.
      if (actionType === "select" && !this.isMeaningfulValue(act.value))
        continue;
