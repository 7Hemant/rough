...
/**
 * A value is replayable only if it represents a real choice. Empty strings and
 * dropdown placeholders ("Select", "-- Select --", "Choose...") must NEVER be
 * pushed into a field during replay:
 *  - smartFill("") actively BLANKS OUT a field the app prefilled;
 *  - smartSelect("") matches the placeholder <option value=""> and RESETS a
 *    dropdown the app (or an earlier step / "Same as ..." copy) already set —
 *    which is how a filled State selector ends up empty right before Next.
 */
function isReplayableValue(value: string): boolean {
...
async function smartFill(
  ...
  // Safety net: never fill an empty value — it would erase whatever the app
  // prefilled in the field. Callers should have skipped the step already.
...
async function smartSelect(
  ...
  // Safety net: an empty/placeholder value would match the placeholder
  // <option value=""> below and RESET the dropdown (this is exactly how a
  // filled State select gets emptied). Refuse to do that.
  if (!isReplayableValue(value)) return;
  ...
  if (tagName === "select") {
    ...
    const applied = await handle.evaluate((el: any, val: string) => {
      ...
      if (!target) return false; // never match the placeholder option
      const options = Array.from(el.options) as any[];
      const match =
        options.find((o) => o.value === val && o.value !== "") ||
  ...
  async executeWithDynamicFields(opts: {
    ...
    try {
      ...
      for (const field of fields) {
        ...
        // FIX (filled field turning empty before Next): a fill/select step with
        // an empty or placeholder value doesn't "do nothing" — it actively
        // CLEARS the field (fill("") blanks it; select matches the placeholder
        // <option value="">). Legacy recordings contain such steps (programmatic
        // changes captured at page-init / "Same as Permanent Address" copy /
        // submit-time resets), so if one targets e.g. the State dropdown AFTER
        // it was set, the value visibly reverts to "Select" and validation
        // fails. Skip these steps entirely — never write an empty value.
        if (
          ["fill", "select"].includes(field.actionType) &&
          !isReplayableValue(value)
        ) {
          logger.warn(
            stepNum,
            `Skipping "${field.label}" — no real value to ${field.actionType} ` +
              `(got "${value}"). Writing it would clear/reset the field.`,
            { details: `selector: ${field.selector}` },
          );
          results.push({
            step: stepNum,
            field: field.label,
            action: `SKIPPED (empty/placeholder value for ${field.actionType})`,
            status: "SKIPPED",
          });
          continue;
        }
