 ...
 *    inputs etc.).
 * Callers act on the RETURNED HANDLE, not by re-querying the selector, so
...
async function smartFill(ctx: any, selector: string, value: string, opts?: { timeout?: number }) {
  ...
  // step panels. Fall back to the first attached match (readonly datepicker /
  // hidden input path below handles it via JS).
  let el: any = null;
  try {
  ...
  if (!isReadonly && isVisible) {
    try {
      ...
      try { await el.press('Escape'); } catch {}
      return;
    } catch {
 ...
 *     failed, we drive the widget through its UI: click the .dropdown-toggle button,
 *     type into the live-search box (.bs-searchbox input), and click the option.
 *  3. Fully custom dropdown — opens the widget, types into its search box if present,
 *     and clicks the option whose text matches.
 *
 * CASCADE FIX (dropdowns that depend on a previous field): dependent dropdowns
 * (State → City) get their options via AJAX triggered by the previous select's
 * change handler. When replay reaches the dependent select, its options may not
 * have arrived yet — so Case 2 now RETRIES matching the option for up to 12s
 * instead of giving up on the first look. Combined with the networkidle wait
 * after every applied selection, a full State → City → Area chain replays
 * reliably even on slow servers.
 */
async function smartSelect(ctx: any, page: any, selector: string, value: string, opts?: { timeout?: number }) {
  ...
  if (tagName === 'select') {
    ...
    // Case 1: plain visible native select. Playwright's selectOption internally
    // retries until the option exists, so late AJAX-loaded options are covered
    // by its own timeout.
    if (isVisible) {
    ...
    // selectpicker). Match the option by value OR visible text (recorder stores
    // the visible text), set it on the real <select>, then notify BOTH native
    // and jQuery listeners AND call each widget's refresh API so its visible
    // label updates.
    //
    // CASCADE FIX: the match attempt RETRIES for up to 12s — a dependent
    // dropdown's options arrive via AJAX after the previous select's change,
    // and on slow servers they are not present on the first look.
    const tryApply = () =>
    ...
    if (!applied) {
      ...
      while (!applied && Date.now() < matchDeadline) {
        try { await pg.waitForTimeout(500); } catch {}
    ...
    if (applied) {
      ...
      // wait for the network to actually go idle (bounded) so the NEXT
      // dependent field's options are loaded before its step runs.
    ...
    // <select> even after waiting (options may load only when the dropdown
    // opens, or the recorded text differs slightly) — operate the widget the
    // way a user does: open the toggle button, type in the live-search box,
    // click the option.
 ...
 *
 * Multi-step wizard handling: when the CSS selector matches MULTIPLE elements
 * (one per step panel), we check whether exactly one of them is VISIBLE — if so,
 * the CSS selector is still usable because every action helper resolves to the
 * first visible match. Otherwise we fall back to the recorded XPath, which is
 * indexed and points at the exact element from the recording.
 */
