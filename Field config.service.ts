...
export class FieldConfigService {
  ...
  private collapseSelect2Cascades(
    ...
    //
    // FIX (bootstrap-select live search): also drop artifacts from bootstrap-select
    // recordings — the live-search input (recorded as [aria-label="Search"] or a
    // plain input inside .bs-searchbox), the toggle button (button.dropdown-toggle /
    // .btn.dropdown-toggle), option anchors (a.dropdown-item, li a inside the menu),
    // and the .bootstrap-select wrapper itself. These appear in legacy recordings made
    // before the recorder suppressed widget internals; without this cleanup they
    // become unreplayable steps (the elements don't exist while the menu is closed).
    const isTransientInternalSelector = (s: string, label: string) =>
      ...
      /(^|\s)a\.dropdown-item\b/i.test(s) ||
      /\.dropdown-item\b/i.test(s) ||
      /aria-label="?search"?/i.test(s) ||
      /(^|\s)li\.(selected|active)\b/i.test(s) ||
