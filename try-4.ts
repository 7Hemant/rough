...
/**
 * FIX (field populated initially but cleared afterward): after a successful fill,
 * some stacks wipe the value a moment later —
 *  - a cascading dropdown elsewhere on the form fires AJAX and the response
 *    re-renders/replaces this panel (the freshly-rendered input is empty);
 *  - jQuery validation / widget plugins reset the input during their own
 *    change/blur cycle;
 *  - datepicker widgets re-write the input when their popup closes.
 *
 * This helper re-checks the input a few times AFTER the fill and re-applies the
 * value (via JS + input/change/jQuery events) whenever it comes back EMPTY.
 * It re-resolves the element on every check because a re-render may have replaced
 * the original node, making the old handle stale.
 *
 * Non-empty but reformatted values (input masks, datepickers normalizing
 * "01/02/2026" → "1/2/2026") are accepted as-is so we never fight a mask.
 */
async function verifyFillPersisted(
  ctx: any,
  selector: string,
  value: string,
  attempts = 3,
  ...
  for (let i = 0; i < attempts; i++) {
    ...
    // Re-resolve the element — the node may have been replaced by a re-render
    let el: any = null;
    try {
      el = await firstVisibleElement(ctx, selector, 1500);
    } catch {
      el = await ctx.$(selector).catch(() => null);
    }
    if (!el) return;

    const current = await el
      .evaluate((n: any) => ("value" in n ? n.value : n.textContent) ?? "")
      .catch(() => null);
    if (current === null) continue; // handle went stale mid-check — retry
    if (String(current).trim().length > 0) return; // value stuck — done

    // Value was CLEARED — re-apply it via JS and notify all listeners
    await el
      .evaluate((node: any, val: string) => {
 ...
 *  - FIX: values that get CLEARED after filling (cascade re-renders, validators) —
 *    we now wait for in-flight AJAX to finish BEFORE filling, and VERIFY the value
 *    persisted afterward, re-applying it if it was wiped (see verifyFillPersisted).
 */
async function smartFill(
  ...
  const timeout = opts?.timeout || 20000;
  const pg = pageOf(ctx, null);

  // FIX (value cleared after fill): if a previous step (e.g. a cascading dropdown)
  // fired AJAX that re-renders this panel, filling too early means the re-render
  // replaces the input and the value vanishes. Let in-flight requests settle first.
  try {
    await pg.waitForLoadState("networkidle", { timeout: 5000 });
  } catch {}
  ...
  if (!isReadonly && isVisible) {
    try {
      ...
      // FIX: confirm the value survived widget/validator/cascade activity;
      // re-apply it if something cleared it
      await verifyFillPersisted(ctx, selector, value);
      return;
    } catch {
  ...
  // FIX: same persistence guarantee for the JS-assignment path
  await verifyFillPersisted(ctx, selector, value);
}

/**
 * FIX (cascading dropdowns): resolve the "active" select handle for a selector.
 *
 * Extracted from smartSelect so it can be called REPEATEDLY — when a parent
 * dropdown's cascade reloads a dependent dropdown, the AJAX response often
 * REPLACES the <select> node, making any previously-captured handle stale.
 *
 * Among multiple matches (wizard forms), picks the one that is "active" — either
 * the select itself is visible, or (enhanced selects are display:none) its widget
 * wrapper (.bootstrap-select / .select2 / .chosen-container) is visible.
 * Falls back to the first match. Returns null when nothing matches.
 */
async function resolveActiveSelectHandle(
  ctx: any,
  selector: string,
): Promise<any> {
  try {
...
/**
 * FIX (cascading dropdowns): wait until an <option> matching `value` actually
 * EXISTS in the select.
 *
 * Dependent dropdowns (State → City) load their options via AJAX after the parent
 * changes. If we try to select "Delhi" before the options arrive, either the
 * selection fails outright, or it "succeeds" and is then wiped when the freshly
 * loaded options replace the list. Polling for the option first eliminates both
 * races. Re-resolves the handle each poll because the reload may replace the node.
 *
 * Matches by option value, exact visible text, then substring — same rules the
 * apply step uses. Returns true when found, false on timeout (callers fall through
 * to UI-driven fallbacks).
 */
async function waitForSelectOption(
  ...
  while (Date.now() < deadline) {
    try {
      const els = await ctx.$$(selector);
      for (const el of els) {
        const found = await el
          .evaluate((node: any, val: string) => {
            if (!node || !node.options) return false;
            const norm = (s: string) =>
              (s || "").replace(/\s+/g, " ").trim().toLowerCase();
            const target = norm(val);
            const options = Array.from(node.options) as any[];
            return options.some(
              (o) =>
                o.value === val ||
                norm(o.textContent) === target ||
                (target.length > 0 && norm(o.textContent).includes(target)),
            );
          }, value)
          .catch(() => false);
...
/**
 * Apply a value to a (possibly hidden) native <select>:
 ...
 * Returns true when a matching option was found and applied.
 */
async function applySelectValue(handle: any, value: string): Promise<boolean> {
  return await handle
...
/**
 * FIX (cascading dropdowns): check whether the select's CURRENT selection still
 * corresponds to the requested value (by option value, exact text, or substring).
 */
async function isSelectValueApplied(
  ...
  return await handle
    .evaluate((el: any, val: string) => {
      ...
      if (!sel) return false;
      // An empty/placeholder selection means the widget was reset
      if (!sel.value && norm(sel.textContent).length === 0) return false;
      return (
        sel.value === val ||
        norm(sel.textContent) === target ||
        (target.length > 0 && norm(sel.textContent).includes(target))
      );
    }, value)
    .catch(() => false);
}
...
/**
 * FIX (cascading dropdowns): after a selection, VERIFY it survived and re-apply
 * it if an options reload reset it.
 *
 * Scenario: our selection (or a parent dropdown's earlier change) triggers AJAX
 * that re-populates this dropdown's option list. When the new options land, the
 * browser resets the selection to the placeholder — so the value we set a moment
 * ago silently disappears. This waits for the cascade network activity to finish,
 * re-resolves the (possibly replaced) select node, checks the selection, and
 * re-applies the value until it sticks (bounded retries).
 */
async function ensureSelectValueSticks(
  ctx: any,
  pg: any,
  selector: string,
  value: string,
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    // Let the cascade's AJAX (options reload) finish before judging the state
    try {
      await pg.waitForLoadState("networkidle", { timeout: 8000 });
    } catch {}
    // FIX (State cleared AFTER passing verification): many apps reset fields from
    // a deferred setTimeout/success-callback that runs AFTER the network goes
    // idle. Checking immediately at idle sees the value still applied — and then
    // the deferred handler wipes it. Give those handlers a moment to run first.
    await new Promise((r) => setTimeout(r, 400));

    const h = await resolveActiveSelectHandle(ctx, selector);
    if (!h) return;
    const tag = await h
    ...
    if (await isSelectValueApplied(h, value)) return; // selection stuck — done
    if (attempt === retries) return; // out of retries — leave last state

    // Selection was RESET by an options reload — wait for the option to be
    // available again, then re-apply on a FRESH handle (node may be new).
    await waitForSelectOption(ctx, selector, value, 8000);
    const h2 = await resolveActiveSelectHandle(ctx, selector);
    if (h2) await applySelectValue(h2, value);
  }
}

/**
 * A dropdown value applied during the run that must SURVIVE the rest of the run.
 */
interface StickySelect {
  ctx: any;
  selector: string;
  value: string;
  label: string;
...
/**
 * FIX (State populated initially, cleared LATER in the run): per-step verification
 * can't catch this — the State step passes, and only a LATER action wipes it:
 *  - selecting the dependent City fires a change handler that re-syncs/resets the
 *    parent State (or re-renders the whole address block);
 *  - a Pin Code lookup response lands late and repopulates/clears State/City;
 *  - a "Same as Permanent Address" toggle copies the section and resets fields
 *    whose options weren't loaded yet at copy time;
 *  - deferred setTimeout handlers reset fields AFTER the network goes idle.
 *
 * So we keep a registry of every dropdown value applied during the run and, after
 * EVERY subsequent successful step, re-check each one and RE-APPLY any that the
 * page reset (waiting for its option to exist again first — a reload may have
 * emptied the option list too).
 *
 * Entries are processed in the order they were applied (parent State before
 * dependent City), so re-applying a parent — whose change reloads the child —
 * is immediately followed by the child's own re-check and re-apply in the same
 * pass. One pass per step keeps this cheap and cycle-free.
 */
async function reassertStickySelects(
  page: any,
  sticky: StickySelect[],
  ...
  for (const s of sticky) {
    try {
      const h = await resolveActiveSelectHandle(s.ctx, s.selector);
      if (!h) continue;
      const tag = await h
        .evaluate((el: any) => el.tagName.toLowerCase())
        .catch(() => "");
      ...
      if (await isSelectValueApplied(h, s.value)) continue; // still set — OK

      // The page reset this dropdown after its step already passed — restore it
      logger.warn(
        stepNum,
        `"${s.label}" was cleared by the page after being set — re-applying "${s.value}"`,
        { details: `selector: ${s.selector}` },
      );
      await waitForSelectOption(s.ctx, s.selector, s.value, 8000);
      const h2 = await resolveActiveSelectHandle(s.ctx, s.selector);
      if (h2 && (await applySelectValue(h2, s.value))) {
        // Re-applying fires the change handler → possible cascade AJAX for its
        // dependent dropdown; let it finish before the next entry is checked.
        try {
          await pageOf(s.ctx, page).waitForLoadState("networkidle", {
            timeout: 8000,
          });
        } catch {}
        logger.action(stepNum, `Restored "${s.label}" = "${s.value}"`, {
          selector: s.selector,
        });
      }
    } catch {}
...
/**
 * FIX (State STILL clearing between/after Node-side checks): install a guard that
 * lives INSIDE the page and defends the select continuously.
 *
 * Node-side verification (ensureSelectValueSticks / reassertStickySelects) only
 * observes the page at discrete moments — a repeating or deferred page handler
 * (interval-based form sync, "Same as Permanent" copier, late AJAX callback) can
 * clear the field in the gaps between those checks, or after the last one. This
 * guard closes the gaps:
 *
 *  - A 500ms interval inside the browser re-finds the select (survives node
 *    replacement), checks whether the desired value is still applied, and
 *    RE-APPLIES it immediately when cleared (native events + jQuery change +
 *    Select2/selectpicker/Chosen refresh). Bounded by duration and a max
 *    re-apply count so it can never loop forever against the app.
 *  - DIAGNOSTICS: patches HTMLSelectElement's `value` and `selectedIndex`
 *    setters so that when ANY page script clears a guarded select, the guard
 *    records a STACK TRACE of the caller into window.__selectGuardLog. A
 *    MutationObserver also records option-list replacements (innerHTML swaps
 *    that reset the selection without touching the setters). Drain the log with
 *    drainSelectGuardLog — the stack shows exactly WHICH app script clears the
 *    field.
 *
 * Re-installing for the same selector replaces the previous guard (latest value
 * wins). Guards die naturally on navigation, which is fine — the form is gone.
 */
async function installSelectValueGuard(
  ctx: any,
  selector: string,
  value: string,
  opts?: { durationMs?: number; maxReapplies?: number },
) {
  const durationMs = opts?.durationMs ?? 90000;
  const maxReapplies = opts?.maxReapplies ?? 10;
  try {
    await ctx.evaluate(
      (args: {
        sel: string;
        ...
        const { sel, val, durationMs, maxReapplies } = args;
        const w = window as any;
        w.__selectGuards = w.__selectGuards || {};
        w.__selectGuardLog = w.__selectGuardLog || [];
        const log = (entry: any) => {
          try {
            w.__selectGuardLog.push({ t: new Date().toISOString(), ...entry });
            if (w.__selectGuardLog.length > 200) w.__selectGuardLog.shift();
          } catch {}
        };

        // --- one-time setter patches to capture WHO clears guarded selects ---
        if (!w.__selGuardPatched) {
          try {
            const proto = HTMLSelectElement.prototype as any;
            const dVal = Object.getOwnPropertyDescriptor(proto, "value");
            if (dVal && dVal.set && dVal.get) {
              Object.defineProperty(proto, "value", {
                get: dVal.get,
                set: function (v: any) {
                  try {
                    if (this.__guarded && (v === "" || v == null)) {
                      log({
                        selector: this.__guardSel,
                        action: "cleared-by (value setter)",
                  ...
                  return dVal.set!.call(this, v);
                },
                configurable: true,
              });
            }
            const dIdx = Object.getOwnPropertyDescriptor(
              proto,
              "selectedIndex",
            );
            if (dIdx && dIdx.set && dIdx.get) {
              Object.defineProperty(proto, "selectedIndex", {
                get: dIdx.get,
                set: function (v: any) {
                  try {
                    if (this.__guarded && (v === -1 || v === 0)) {
                      log({
                        selector: this.__guardSel,
                        action: "cleared-by (selectedIndex setter)",
                        stack: (new Error().stack || "")
                          .split("\n")
                          .slice(2, 7)
                          .map((s: string) => s.trim())
                  ...
                  return dIdx.set!.call(this, v);
                },
                configurable: true,
              });
            }
            w.__selGuardPatched = true;
          } catch {}
        }
        ...
        const norm = (s: string) =>
          (s || "").replace(/\s+/g, " ").trim().toLowerCase();
        const target = norm(val);
        const isApplied = (el: any) => {
          if (!el || !el.options) return false;
          const o = el.options[el.selectedIndex];
          if (!o) return false;
          if (!o.value && norm(o.textContent).length === 0) return false;
          return (
            o.value === val ||
            norm(o.textContent) === target ||
            (target.length > 0 && norm(o.textContent).includes(target))
        ...
        const apply = (el: any): boolean => {
          const options = Array.prototype.slice.call(el.options) as any[];
          const match =
            options.find((o) => o.value === val) ||
            options.find((o) => norm(o.textContent) === target) ||
            options.find(
        ...
        const markGuarded = (els: any[]) => {
          for (const el of els) {
            if (!el || el.tagName !== "SELECT") continue;
            if (el.__guarded && el.__guardSel === sel) continue;
            el.__guarded = true;
            el.__guardSel = sel;
            // Log option-list replacements (innerHTML swaps reset the selection
            // without ever touching the value/selectedIndex setters)
            try {
              const mo = new MutationObserver(() => {
                log({
                  selector: sel,
                  action: "options-replaced",
        ...
        // Replace any previous guard for this selector — latest value wins
        if (w.__selectGuards[sel]) {
          try {
            clearInterval(w.__selectGuards[sel]);
          } catch {}
        }
        markGuarded(findEls());
        let reapplies = 0;
        ...
        const timer = setInterval(() => {
          try {
            if (
              ...
              reapplies >= maxReapplies
            ) {
              clearInterval(timer);
              delete w.__selectGuards[sel];
              return;
            }
            const els = findEls();
            markGuarded(els); // re-mark: the node may have been replaced
            for (const el of els) {
              if (!el || el.tagName !== "SELECT") continue;
              if (isApplied(el)) return; // one match holds the value — all good
            }
            for (const el of els) {
              if (!el || el.tagName !== "SELECT") continue;
              if (apply(el)) {
                ...
                log({
                  selector: sel,
                  value: val,
                  action: "guard-reapplied",
                  count: reapplies,
                });
                return;
              }
            }
            // Option not present (list emptied by a reload) — the next tick
            // will retry once the options come back.
          } catch {}
        }, 500);
        w.__selectGuards[sel] = timer;
        log({ selector: sel, value: val, action: "guard-installed" });
      },
      { sel: selector, val: value, durationMs, maxReapplies },
    );
  } catch {}
}

/**
 * Drain the in-page guard's diagnostic log (and clear it). Returns entries like
 * guard-installed / guard-reapplied / options-replaced / cleared-by (+ stack).
 */
async function drainSelectGuardLog(target: any): Promise<any[]> {
  try {
    return await target.evaluate(() => {
      const w = window as any;
      const entries = w.__selectGuardLog || [];
      w.__selectGuardLog = [];
      return entries;
    });
  } catch {
...
/**
 * Pull guard diagnostics out of the page and surface them in the execution log —
 * "cleared-by" entries include the STACK of the app script doing the clearing,
 * which is the fastest way to identify the culprit handler.
 */
async function emitGuardDiagnostics(
  targets: any[],
  logger: ExecutionLogger,
  stepNum: number,
) {
  ...
  for (const t of targets) {
    ...
    seen.add(t);
    const entries = await drainSelectGuardLog(t);
    for (const e of entries) {
      if (e.action === "guard-reapplied") {
        logger.warn(
          stepNum,
          `Page cleared "${e.selector}" — in-page guard restored "${e.value}" (restore #${e.count})`,
        );
      } else if (
        typeof e.action === "string" &&
        e.action.indexOf("cleared-by") === 0
      ) {
        logger.warn(stepNum, `Select "${e.selector}" cleared by page script`, {
          details: e.stack || e.action,
        });
      } else if (e.action === "options-replaced") {
        logger.info(
          stepNum,
          `Option list replaced for "${e.selector}" (${e.optionCount} options)`,
        );
      }
    }
  }
 ...
 *
 * FIX (dependent/cascading dropdowns — State → City):
 *  - BEFORE selecting: wait for network idle (the parent's cascade may still be
 *    loading this dropdown's options) and poll until the requested option actually
 *    EXISTS (waitForSelectOption), then re-resolve the handle because the AJAX
 *    reload frequently REPLACES the <select> node.
 *  - AFTER selecting: verify the selection survived the reload and re-apply it if
 *    it was reset (ensureSelectValueSticks). This is what stops the second
 *    dropdown from losing its value when the first dropdown's change reloads it.
 */
async function smartSelect(
  ...
  // wrapper is visible. Falls back to the first match. (Extracted to
  // resolveActiveSelectHandle so it can be re-run after cascade reloads.)
  let handle: any = await resolveActiveSelectHandle(ctx, selector);
  ...
  if (tagName === "select") {
    // FIX (cascading dropdowns): a previous dropdown's change handler may still be
    // loading THIS dropdown's options via AJAX. Wait for the network to settle,
    // then wait (bounded) for an option matching the requested value to exist
    // before attempting to select it — selecting too early either fails or gets
    // wiped when the freshly loaded options replace the list.
    try {
      await pg.waitForLoadState("networkidle", { timeout: 8000 });
    } catch {}
    await waitForSelectOption(ctx, selector, value, Math.min(timeout, 15000));
    // The AJAX reload may have REPLACED the node — re-resolve the handle so we
    // don't operate on a stale element.
    handle = (await resolveActiveSelectHandle(ctx, selector)) || handle;
    ...
    if (isVisible) {
      try {
        ...
        // FIX: make sure a late options reload doesn't reset the selection
        await ensureSelectValueSticks(ctx, pg, selector, value);
        return;
      } catch {}
      try {
    ...
    // Case 2: enhanced select whose native element is hidden (Select2 / Chosen /
    // selectpicker). Apply the value on the real <select> and notify native +
    // jQuery listeners + widget refresh APIs (see applySelectValue).
    const applied = await applySelectValue(handle, value);
    if (applied) {
      ...
      // FIX: if that AJAX (or a racing reload) reset this dropdown's selection,
      // re-apply it until it sticks.
      await ensureSelectValueSticks(ctx, pg, selector, value);
      return;
    }
      ...
      try {
        ...
        for (const optSel of bsOptionSelectors) {
          ...
          if (opt && (await opt.isVisible().catch(() => false))) {
            ...
            // FIX: verify the UI-driven selection also survived any reload
            await ensureSelectValueSticks(ctx, pg, selector, value);
            return;
          }
        }
  ...
  for (const optSel of optionSelectors) {
    ...
    if (await opt.count().catch(() => 0)) {
      try {
        ...
        // FIX: if the widget is backed by a native <select>, confirm the value
        // stuck (ensureSelectValueSticks no-ops for non-select elements)
        await ensureSelectValueSticks(ctx, pg, selector, value);
        return;
      } catch {}
    }
...
export class DynamicStepService {
  ...
  async executeWithDynamicFields(opts: {
    ...
    // FIX (State cleared later in the run): registry of dropdown values applied
    // so far — re-verified and restored after every subsequent successful step
    // (see reassertStickySelects).
    const stickySelects: StickySelect[] = [];
    let stepNum = 0;
        ...
        try {
          ...
          switch (field.actionType) {
            ...
            case "select":
              ...
              // FIX: remember this selection so it can be restored if a LATER
              // step (dependent dropdown, pincode lookup, copy-address toggle)
              // clears it. Replace any earlier entry for the same selector.
              {
                const existing = stickySelects.findIndex(
                  (s) => s.selector === resolvedSel,
                );
                const entry: StickySelect = {
                  ctx,
                  selector: resolvedSel,
                  value,
                  label: field.label,
                };
                if (existing >= 0) stickySelects[existing] = entry;
                else stickySelects.push(entry);
              }
              // FIX (still clearing between Node-side checks): plant a guard
              // INSIDE the page that continuously restores this value the moment
              // any page script clears it, and records the clearing script's
              // stack trace for diagnostics (see installSelectValueGuard).
              await installSelectValueGuard(ctx, resolvedSel, value);
              break;
            case "check": {
              logger.wait(
                stepNum,
          ...
          // FIX (State populated then cleared later): this step's action may have
          // triggered a handler (City cascade, pincode lookup, copy-address
          // toggle, deferred reset) that wiped a dropdown set in an EARLIER step.
          // Re-verify every previously applied dropdown and restore any that were
          // cleared before moving on.
          if (stickySelects.length > 0) {
            await reassertStickySelects(page, stickySelects, logger, stepNum);
            // Surface in-page guard activity (restores + WHO cleared the field)
            await emitGuardDiagnostics(
              [page, ctx].filter(Boolean),
              logger,
              stepNum,
            );
          }
