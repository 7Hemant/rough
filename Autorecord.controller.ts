    ...
    (async () => {
      try {
        ...
        await context.addInitScript(() => {
          ...
          // ================================================================
          // FIX (record ONLY fields the user actually interacted with):
          //
          // A field must become a step only when a VALUE-CHANGING action
          // happens on it AND that change is traceable to a real user
          // gesture. Previously untouched fields leaked into recordings via:
          //   1. Programmatic change events on page load — jQuery apps init
          //      widgets with $('select').trigger('change') (cascade setup,
          //      selectpicker render). The delegated jQuery listener recorded
          //      them even though the user did nothing.
          //   2. Clicking INTO a text field without typing — the click
          //      listener recorded a click step on the input itself.
          //   3. The focus listener — tabbing through fields / auto-focus on
          //      page load recorded a step for every field focus touched.
          //   4. Hover dwell — resting the mouse over a field while reading
          //      the form recorded a hover step on it.
          //
          // The mechanisms below fix all four:
          //   - A "user gesture" timestamp (mousedown/keydown/touchstart).
          //     recordChange() rejects changes with no recent gesture, which
          //     filters ALL page-load / programmatic init changes.
          //   - Select changes carrying a placeholder ("Select", "Choose...")
          //     or empty value are skipped — a real user choice always fires
          //     again with the real option text (cascade resets are filtered).
          //   - Clicks on text-entry controls (text inputs, textareas,
          //     selects) are NOT recorded — the change event captures the
          //     real action; click-without-typing records nothing.
          //   - The focus listener is removed.
          //   - Hovers that resolve to form fields / labels are skipped.
          // ================================================================
          ...
          // A change event is user-initiated only if a real gesture happened
          // recently. 3s window: covers slow widgets (Select2 search + click)
          // while filtering page-load init changes (no gesture yet).
          function hasRecentUserGesture(): boolean {
            return Date.now() - lastUserGestureTime < 3000;
          }

          // Text-entry / value controls whose clicks should NOT be recorded —
          ...
          // Placeholder texts a <select> reports before a real user choice.
          function isPlaceholderChoice(text: string): boolean {
            const v = (text || "").replace(/\s+/g, " ").trim().toLowerCase();
            if (!v) return true;
            return [
              "select",
              "-- select --",
          ...
          document.addEventListener(
            ...
            (e: any) => {
              ...
              // FIX (only record acted-on fields): do NOT record a click on a
              // text-entry control (text input / textarea / select). Clicking into
              // a field is not an action — if the user actually types or picks a
              // value, the change event records the real fill/select/check step.
              // If they click in and type nothing, NOTHING is recorded.
              // Clicks on buttons, links, checkboxes and radios are still recorded.
          ...
          function recordChange(el: any) {
            ...
            // FIX (only record acted-on fields): reject changes that are NOT
            // traceable to a recent real user gesture. jQuery apps fire
            // $('select').trigger('change') on page load to initialize cascades
            // and selectpickers — those programmatic changes used to create
            // steps for fields the user never touched.
            ...
            if (el.tagName === "SELECT") {
              ...
              // FIX (only record acted-on fields): skip placeholder / empty
              // selections. Cascade handlers reset dependent dropdowns to
              // "Select..." right after a parent choice — within the gesture
              // window but NOT a user action on that field. A real choice fires
              // change again with the real option text and IS recorded.
              if (isPlaceholderChoice(recordedValue)) return;
            }

            // FIX (only record acted-on fields): skip empty text fills — a change
            // with an empty value on a text control is a blur/reset artifact, not
            // an intentional entry. (Deliberately clearing a prefilled field is
            // rare; add it manually in Field Manager if a test needs it.)
          ...
          // Skip changes coming from widget internals (bs-searchbox input etc.) —
          ...
          // (Programmatic init triggers are filtered inside recordChange via the
          // user-gesture window.)
              ...
              try {
                ...
                jq(document)
                  ...
                  .on("change.__recorder", "select, input, textarea", function (
                    ...
                    // Same guard as the native listener — ignore the live-search
          ...
          document.addEventListener(
            ...
            (e: any) => {
              ...
              // FIX (only record acted-on fields): never record hovers over form
              // fields or their labels. Resting the mouse on a field while reading
              // the form used to create a hover step for a field the user never
              // filled. Hovers remain recorded for menus / nav / tooltip triggers.
              if (
          ...
          // FIX (only record acted-on fields): the FOCUS listener has been REMOVED.
          // It recorded a step for every INPUT/SELECT/TEXTAREA that received focus —
          // including tab-through navigation and auto-focus on page load — and those
          // steps replayed as clicks on fields the user never actually used. Real
          // interactions are fully covered by the change (fill/select/check) events.
          ...
          document.addEventListener(
            ...
            (e: any) => {
              ...
              // select a word before retyping) is not a step — the change event
          ...
          // KEYDOWN events for special keys (Enter, Escape).
          // FIX (only record acted-on fields): Tab is no longer recorded — tabbing
          // between fields is navigation, not an action, and it generated a press
          // step for every field the user merely passed through.
          document.addEventListener(
            "keydown",
            (e: any) => {
              if (["Enter", "Escape"].includes(e.key)) {
                ...
                // box selects the highlighted option — the resulting <select>
          ...
          document.addEventListener(
            ...
            (e: any) => {
              ...
              if (isTransientWidgetInternal(el)) return;
              el = getInteractiveElement(el);
              // Right-clicking a text field (paste menu) is not a step either —
